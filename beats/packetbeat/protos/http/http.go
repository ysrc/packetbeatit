...
func (http *HTTP) setFromConfig(config *httpConfig) {
	...
	http.SpecifyReferer = config.Specify_referer
	http.SpecifyHost = config.Specify_host
...
func (http *HTTP) correlate(conn *httpConnectionData) {
	...
	for !conn.responses.empty() && !conn.requests.empty() {
		...
		if len(http.SpecifyHost) > 0 && !http.filterHost(requ) {
			debugf("The URL is not in the Specify Host list.")
			return
		}
		if len(http.SpecifyReferer) > 0 && !http.filterReferer(requ) {
			debugf("The URL is not in the Specify Referer list.")
			return
		}
...
func (http *HTTP) filterHost(m *message) bool {

	if len(http.SpecifyHost) > 0 && len(m.Headers["host"]) == 0 {
		return false
	}

	specifyhost := http.SpecifyHost
	host := m.Headers["host"]

	for _, include := range specifyhost {
		if bytes.Contains(host, []byte(include)) {
			if isDebug {
				debugf("Is the right URL  host %s include %s",
					host, include)
			}
			return true
		}
		if isDebug {
			debugf("Not the right URL  host %s include %s",
				host, include)
		}
	}
	return false
...
func (http *HTTP) filterReferer(m *message) bool {
	if len(http.SpecifyReferer) > 0 && len(m.Headers["referer"]) == 0 {
		return false
	}

	specifyreferer := http.SpecifyReferer
	referer := m.Headers["referer"]

	for _, include := range specifyreferer {
		if bytes.Contains(referer, []byte(include)) {
			if isDebug {
				debugf("Is the right URL referer %s include %s",
					referer, include)
			}
			return true
		}
		if isDebug {
			debugf("Not the right URL referer %s include %s",
				referer, include)
		}
	}
	return false
